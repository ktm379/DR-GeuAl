    ...
    def train_on_batch(self, x_batch_train, y_batch_train):
        with tf.GradientTape() as tape:
            ...
            if not self.for_recons:
                ...
                # loss 가중합 해주기
                mask_loss = self.b1 * ex_loss + self.b2 * he_loss + self.b3 * ma_loss + self.b4 * se_loss
                ...
                # loss 시각화하기 위해 전달할 값
                return_loss = (loss_recons.numpy(), train_loss.numpy(), mask_loss.numpy(), ex_loss.numpy(), he_loss.numpy(), ma_loss.numpy(), se_loss.numpy())
    ...
    def train(self, train_dataset, val_dataset):
        ...
        for epoch in range(self.epochs):
            ...
            for step_train, (x_batch_train, y_batch_train) in enumerate(train_dataset):
                if not self.for_recons:
                    loss_recons, train_loss, mask_loss, ex_loss, he_loss, ma_loss, se_loss = self.train_on_batch(x_batch_train, y_batch_train)
                    
                    values = [('train_loss', train_loss), ('mask_loss', mask_loss), ('loss_recons', loss_recons), ('ex_loss', ex_loss), ('he_loss', he_loss), ('ma_loss', ma_loss), ('se_loss', se_loss)]
                    ...
                    he_batch_loss.append(he_loss)
                    ma_batch_loss.append(ma_loss)
                    ...
                    if (step_train + 1) == len(train_dataset):
                        values = [('train_loss', np.mean(total_batch_loss)), 
                                  ...
                                  ('he_loss', np.mean(he_batch_loss)),
                                  ('ma_loss', np.mean(ma_batch_loss)),
            ...
            log = f"epoch:{epoch + self.first_epoch}/"\
                  ...
                  f"he_loss:{np.mean(he_batch_loss)}/"\
                  f"ma_loss:{np.mean(ma_batch_loss)}/"\
            ...
            val_progBar = Progbar(target=len(val_dataset) * val_dataset.batch_size, stateful_metrics=['val_loss','mask_loss', 'loss_recons', 'ex_loss', 'he_loss', 'ma_loss', 'se_loss'])
            ...
            for step_val, (x_batch_val, y_batch_val) in enumerate(val_dataset):
                ...
                # ex, he, ma, se
                    ...
                    he_batch_loss.append(he_loss.numpy())
                    ma_batch_loss.append(ma_loss.numpy())
