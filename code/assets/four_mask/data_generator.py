...
class DR_Generator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        if self.dataset == "FGADR":
            ...
            return [tf.cast(inputs, dtype=tf.float32)], [tf.cast(ex, dtype=tf.float32), tf.cast(he, dtype=tf.float32), tf.cast(ma, dtype=tf.float32), tf.cast(se, dtype=tf.float32)]
