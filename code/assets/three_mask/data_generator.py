...
class DR_Generator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        if self.dataset == "FGADR":
            ...
            ma_he = np.zeros([self.batch_size, *self.img_size])
        ...
        if self.dataset == "FGADR":
            ...
            # input, target
            # [image], [ex, ma_he, se]
            return [tf.cast(inputs, dtype=tf.float32)], [tf.cast(ex, dtype=tf.float32), tf.cast(ma_he, dtype=tf.float32), tf.cast(se, dtype=tf.float32)]
