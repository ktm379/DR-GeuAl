    ...
    def __getitem__(self, index):
        ...
        for i, data in enumerate(batch_data_paths):
            ...
            if self.use_mask:
                ...
                pathlist.append(input_img_path)
        ...
        if self.dataset == "FGADR":
            ...
            if self.add_noise_std != None:
                return [tf.cast(inputs, dtype=tf.float32)], [tf.cast(noisy_inputs, dtype=tf.float32)], tf.cast(ex, dtype=tf.float32), 
            tf.cast(he, dtype=tf.float32), tf.cast(ma, dtype=tf.float32), tf.cast(se, dtype=tf.float32), tf.cast(mask, dtype=tf.float32), pathlist
            
            return [tf.cast(inputs, dtype=tf.float32)], \
               tf.cast(ex, dtype=tf.float32), tf.cast(he, dtype=tf.float32), \
               tf.cast(ma, dtype=tf.float32), tf.cast(se, dtype=tf.float32), \
               tf.cast(mask, dtype=tf.float32), pathlist
