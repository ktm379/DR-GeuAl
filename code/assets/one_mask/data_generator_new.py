...
class DR_Generator(tf.keras.utils.Sequence):
    ...
    def load_dataset(self):
        ...
        # 이미지 path 지정        
        input_images_paths = [os.path.join(self.dir_path, file) for file in self.file_names]
        input_images_paths.sort()

        # mask path 지정
        # mask : HardExudate, Hemohedge, Microane, SoftExudates
        if self.use_mask:
            ex_paths = [os.path.join(self.mask_path[0], file) for file in self.file_names]
            ex_paths.sort()

            he_paths = [os.path.join(self.mask_path[1], file) for file in self.file_names]
            he_paths.sort()

            me_paths = [os.path.join(self.mask_path[2], file) for file in self.file_names]
            me_paths.sort()

            se_paths = [os.path.join(self.mask_path[3], file) for file in self.file_names]
            se_paths.sort()
